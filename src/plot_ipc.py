...
for line in lines:
    ...
    workload, lq_sq_rob, total_insts, total_cycles, ipc = parts
    ipc = float(ipc)
    all_ipcs.append(ipc)  # Add to list for overall average
    
    match = re.search(r"LQ(\d+)_SQ(\d+)_ROB(\d+)", lq_sq_rob)
    if not match:
        print(f"Skipping invalid LQ_SQ_ROB format: {lq_sq_rob}")
        continue
    
    lq, sq, rob = map(int, match.groups())
    
    config_key = f'LQ{lq}_SQ{sq}_ROB{rob}'
...
all_configs = sorted(list(set(sum([list(w.keys()) for w in workload_configs.values()], []))), key=lambda x: tuple(map(int, re.findall(r'\d+', x))))
...
ax.set_title('IPC Comparison by Scaling up ROB/LQ/SQ Configurations for Datacenter Applications', fontsize=16)
ax.set_xticks(indices + bar_width * (len(all_configs) - 1) / 2)
ax.set_xticklabels(workloads, rotation=45, ha='right', fontsize=12)
ax.legend(title="Configurations (ROB/LQ/SQ)", fontsize=12)
...
plt.savefig('ipc_comparison_grouped.png', dpi=300, bbox_inches='tight')
plt.show()

print("IPC comparison plot saved as ipc_comparison_grouped.png")

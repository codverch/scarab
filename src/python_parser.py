...
def register_instruction(M, mapping, fusable, nopable):
    ...
    if key in mapping:
        ...
        fusable.append((old_inst['counter'], after_fuse_dst_regs, old_inst['num_uops']))
        nopable.append((M['counter'], set(), M['num_uops']))
        del mapping[key]
    else:
        mapping[key] = value
...
def read_struct_from_file(file_name):
    struct_format = 'q q q 8q q 8B B 7x'  # Corresponds to your struct
...
def write_results_to_file(file_name, fusable, nopable):
    struct_format = 'q q 8q b b 6x'
    with open(file_name, 'wb') as file:
        for fusable_inst in tqdm(fusable):
            list_of_reg = list(fusable_inst[1])
            if(len(list_of_reg) < 8):
                list_of_reg += [0] * (8 - len(list_of_reg))
            packed_data = struct.pack(struct_format, fusable_inst[0], len(fusable_inst[1]), *list_of_reg, 0, fusable_inst[2])
            # print("Counter[F] : ", fusable_inst[0])
            file.write(packed_data)
        for nopable_inst in tqdm(nopable):
            packed_data = struct.pack(struct_format, nopable_inst[0], 0, *([0] * 8), 1, nopable_inst[2])

...
def plot_histogram(data, title, xlabel, output_path, bin_edges, show_percentage=True, max_y=None):
    ...
    plt.ylabel(ylabel, fontsize=12, fontweight='bold')  # Made bold
    plt.xlabel(xlabel, fontsize=12, fontweight='bold')  # Made bold
    plt.xticks(range(len(bin_labels)), bin_labels, rotation=45, ha='right')
    
    # Improve Y-axis formatting
    if show_percentage:
        # Format percentage axis with % symbol
        plt.gca().yaxis.set_major_formatter(plt.matplotlib.ticker.PercentFormatter(xmax=100, decimals=0))
    else:
        # Format count axis with thousand separators
        plt.gca().yaxis.set_major_formatter(plt.matplotlib.ticker.StrMethodFormatter('{x:,.0f}'))
    
    # Add more y-ticks for better readability
    plt.gca().yaxis.set_major_locator(plt.matplotlib.ticker.MaxNLocator(nbins=10))
    
    # Add value labels
    max_height = max(values) if len(values) > 0 else 0
    label_height = max_height * 0.01  # Reduced from 0.03 to 0.01 to place labels closer to bars
...
def plot_comparison_side_by_side(workload_data, bin_edges, title, xlabel, output_path, max_y=None):
    ...
    plt.ylabel('Percentage of Micro-op Pairs (%)', fontsize=12, fontweight='bold')
...
def main():
    ...
    for txt_file in glob.glob(os.path.join(args.workload_dir, '*.txt')):
        ...
        inter_title = f'Distance Between Consecutive Pairs Accessing Same Cacheblock: {workload_name}'
